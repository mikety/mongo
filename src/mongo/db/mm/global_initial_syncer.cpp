...
#include "global_initial_syncer.h"
...
MONGO_FAIL_POINT_DEFINE(failGlobalInitialSyncWithBadHost);

// Failpoint which fails initial sync and leaves an oplog entry in the buffer.
MONGO_FAIL_POINT_DEFINE(failGlobalInitSyncWithBufferedEntriesLeft);

// Failpoint which causes the initial sync function to hang before copying databases.
MONGO_FAIL_POINT_DEFINE(globalInitialSyncHangBeforeCopyingDatabases);

// Failpoint which causes the initial sync function to hang before finishing.
MONGO_FAIL_POINT_DEFINE(globalInitialSyncHangBeforeFinish);

// Failpoint which causes the initial sync function to hang before calling shouldRetry on a failed
// operation.
MONGO_FAIL_POINT_DEFINE(globalInitialSyncHangBeforeGettingMissingDocument);

// Failpoint which causes the initial sync function to hang before creating the oplog.
MONGO_FAIL_POINT_DEFINE(globalInitialSyncHangBeforeCreatingOplog);

// Failpoint which stops the applier.
MONGO_FAIL_POINT_DEFINE(rsGlobalInitSyncApplyStop);

// Failpoint which causes the initial sync function to hang afte cloning all databases.
MONGO_FAIL_POINT_DEFINE(globalInitialSyncHangAfterDataCloning);
...
MONGO_EXPORT_SERVER_PARAMETER(numGlobalInitialSyncConnectAttempts, int, 10);
...
// instance of GlobalInitialSyncer corresponds to a single initial sync request.
...
bool GlobalInitialSyncer::isActive() const {
    stdx::lock_guard<stdx::mutex> lock(_mutex);
    return _isActive_inlock();
}

bool GlobalInitialSyncer::_isActive_inlock() const {
    return State::kRunning == _state || State::kShuttingDown == _state;
}

Status GlobalInitialSyncer::startup(std::uint32_t initialSyncMaxAttempts) noexcept {
    invariant(initialSyncMaxAttempts >= 1U);

    log() << "GlobalInitialSyncer::startup";
    stdx::lock_guard<stdx::mutex> lock(_mutex);
    switch (_state) {
        case State::kPreStart:
            ...
            break;
        case State::kRunning:
            return Status(ErrorCodes::IllegalOperation, "global initial syncer already started");
        case State::kShuttingDown:
            return Status(ErrorCodes::ShutdownInProgress, "global initial syncer shutting down");
        case State::kComplete:
            return Status(ErrorCodes::ShutdownInProgress, "global initial syncer completed");
    }

    _setUp_inlock(initialSyncMaxAttempts);
...
Status GlobalInitialSyncer::shutdown() {
    stdx::lock_guard<stdx::mutex> lock(_mutex);
    switch (_state) {
        case State::kPreStart:
...
void GlobalInitialSyncer::_cancelRemainingWork_inlock() {
    ...
    _shutdownComponent_inlock(_globalFetcher);
...
void GlobalInitialSyncer::join() {
    stdx::unique_lock<stdx::mutex> lk(_mutex);
    _stateCondition.wait(lk, [this]() { return !_isActive_inlock(); });
}

GlobalInitialSyncer::State GlobalInitialSyncer::getState_forTest() const {
    stdx::lock_guard<stdx::mutex> lk(_mutex);
    return _state;
}

bool GlobalInitialSyncer::_isShuttingDown() const {
    stdx::lock_guard<stdx::mutex> lock(_mutex);
    return _isShuttingDown_inlock();
}

bool GlobalInitialSyncer::_isShuttingDown_inlock() const {
    return State::kShuttingDown == _state;
}

std::string GlobalInitialSyncer::getDiagnosticString() const {
    LockGuard lk(_mutex);
    str::stream out;
    out << "GlobalInitialSyncer - TBD status";
    /*
        << " opts: " << _opts.toString() << " oplogFetcher: " << _globalFetcher->toString()
...
BSONObj GlobalInitialSyncer::_getInitialSyncProgress_inlock() const {
    try {
        BSONObjBuilder bob;
...
void GlobalInitialSyncer::setStartCollectionClonerFn(
    const StartCollectionClonerFn& startCollectionCloner) {
    LockGuard lk(_mutex);
    _startCollectionClonerFn = startCollectionCloner;
}

void GlobalInitialSyncer::_setUp_inlock(std::uint32_t initialSyncMaxAttempts) {
...
void GlobalInitialSyncer::_tearDown_inlock(const StatusWith<OpTime>& lastApplied) {
    ...
    */
    ...
    _storage->setInitialDataTimestamp(opCtx->getServiceContext(),
                                      lastApplied.getValue().getTimestamp());
    ...
    */
    ...
    if (currentLastAppliedOpTime.isNull()) {
        ...
        invariant(currentLastAppliedOpTime == lastApplied.getValue());
    }
...
void GlobalInitialSyncer::_startInitialSyncAttemptCallback(
    ...
    log() << "Starting initial sync (attempt " << (initialSyncAttempt + 1) << " of "
          << initialSyncMaxAttempts << "), sync port: " << CurrentSyncSourcePort;
    */
    ...
    auto finishInitialSyncAttemptFn = [this](const StatusWith<OpTime>& lastApplied) {
    ...
    */
    ...
    // _syncSource = HostAndPort("localhost", CurrentSyncSourcePort);
    ...
    std::uint32_t chooseSyncSourceMaxAttempts =
        static_cast<std::uint32_t>(numGlobalInitialSyncConnectAttempts.load());
     ...
     */
    _globalFetcher = stdx::make_unique<GlobalFetcher>(
        _exec,
        beginFetchingOpTime,
        _syncSource,
        _opts.remoteOplogNS,
...
/*
void GlobalInitialSyncer::_chooseSyncSourceCallback(
    const executor::TaskExecutor::CallbackArgs& callbackArgs,
    ...
    // GlobalInitialSyncer::shutdown() or the task executor shutting down.
    auto status =
...
Status GlobalInitialSyncer::_truncateOplogAndDropReplicatedDatabases() {
...
Status GlobalInitialSyncer::_scheduleGetBeginFetchingOpTime_inlock(
    std::shared_ptr<OnCompletionGuard> onCompletionGuard) {
...
void GlobalInitialSyncer::_rollbackCheckerResetCallback(
...
void GlobalInitialSyncer::_getBeginFetchingOpTimeCallback(
...
void GlobalInitialSyncer::_lastOplogEntryFetcherCallbackForBeginApplyingTimestamp(
    ...
    const auto opTimeResult = parseOpTime(result);
    ...
    const auto& lastOpTime = opTimeResult.getValue();
    ...
    _fCVFetcher = stdx::make_unique<Fetcher>(
        ...
        RemoteCommandRetryScheduler::makeRetryPolicy(
            numGlobalInitialSyncOplogFindAttempts.load(),
...
void GlobalInitialSyncer::_fcvFetcherCallback(// const StatusWith<Fetcher::QueryResponse>& result,
                                        std::shared_ptr<OnCompletionGuard> onCompletionGuard,
    ...
    log() << "MultiMaster GlobalSync redundant check for fcv: " << version;
...
/*
void GlobalInitialSyncer::_databasesClonerCallback(const Status& databaseClonerFinishStatus,
                                             std::shared_ptr<OnCompletionGuard> onCompletionGuard) {
...
void GlobalInitialSyncer::_lastOplogEntryFetcherCallbackForStopTimestamp(
    const StatusWith<Fetcher::QueryResponse>& result,
    std::shared_ptr<OnCompletionGuard> onCompletionGuard) {
    OpTime optime;
    {
        ...
        auto&& optimeStatus = parseOpTime(result);
    ...
    // oplog entry to seed the oplog before checking the rollback ID.
    {
        ...
        // override its behavior in tests. See GlobalInitialSyncerReturnsCallbackCanceledAndDoesNot-
        // ScheduleRollbackCheckerIfShutdownAfterInsertingInsertOplogSeedDocument in
        // initial_syncer_test.cpp
        auto status =
            _storage->insertDocument(opCtx.get(),
                                     _opts.localOplogNS,
                                     TimestampedBSONObj{oplogSeedDoc, optime.getTimestamp()},
                                     optime.getTerm());
        if (!status.isOK()) {
            stdx::lock_guard<stdx::mutex> lock(_mutex);
            onCompletionGuard->setResultAndCancelRemainingWork_inlock(lock, status);
...
void GlobalInitialSyncer::_getNextApplierBatchCallback(
    const executor::TaskExecutor::CallbackArgs& callbackArgs,
    std::shared_ptr<OnCompletionGuard> onCompletionGuard) {
    stdx::lock_guard<stdx::mutex> lock(_mutex);
    auto status =
...
void GlobalInitialSyncer::_multiApplierCallback(const Status& multiApplierStatus,
                                          OpTime lastApplied,
...
void GlobalInitialSyncer::_lastOplogEntryFetcherCallbackAfterFetchingMissingDocuments(
    const StatusWith<Fetcher::QueryResponse>& result,
    std::shared_ptr<OnCompletionGuard> onCompletionGuard) {
    stdx::lock_guard<stdx::mutex> lock(_mutex);
    auto status = _checkForShutdownAndConvertStatus_inlock(
...
void GlobalInitialSyncer::_rollbackCheckerCheckForRollbackCallback(
    const RollbackChecker::Result& result, std::shared_ptr<OnCompletionGuard> onCompletionGuard) {
    stdx::lock_guard<stdx::mutex> lock(_mutex);
    auto status = _checkForShutdownAndConvertStatus_inlock(result.getStatus(),
...
*/
void GlobalInitialSyncer::_finishInitialSyncAttempt(const StatusWith<OpTime>& lastApplied) {
...
void GlobalInitialSyncer::_finishCallback(StatusWith<OpTime> lastApplied) {
    ...
    if (MONGO_FAIL_POINT(globalInitialSyncHangBeforeFinish)) {
        // This log output is used in js tests so please leave it.
        log() << "initial sync - initialSyncHangBeforeFinish fail point "
                 "enabled. Blocking until fail point is disabled.";
        while (MONGO_FAIL_POINT(globalInitialSyncHangBeforeFinish) && !_isShuttingDown()) {
            mongo::sleepsecs(1);
        }
    }
    ...
    // before GlobalInitialSyncer::join() returns.
    onCompletion = {};
...
/*
Status GlobalInitialSyncer::_scheduleLastOplogEntryFetcher_inlock(Fetcher::CallbackFn callback) {
    BSONObj query = BSON(
...
void GlobalInitialSyncer::_checkApplierProgressAndScheduleGetNextApplierBatch_inlock(
    ...
    if (_lastApplied.isNull()) {
        ...
        invariant(!_lastApplied.getTimestamp().isNull());
        _scheduleRollbackCheckerCheckForRollback_inlock(lock, onCompletionGuard);
        return;
    }
...
void GlobalInitialSyncer::_scheduleRollbackCheckerCheckForRollback_inlock(
    const stdx::lock_guard<stdx::mutex>& lock,
    std::shared_ptr<OnCompletionGuard> onCompletionGuard) {
    // We should check our current state because shutdown() could have been called before
    // we re-acquired the lock.
...
Status GlobalInitialSyncer::_checkForShutdownAndConvertStatus_inlock(
...
Status GlobalInitialSyncer::_scheduleWorkAndSaveHandle_inlock(
...
Status GlobalInitialSyncer::_scheduleWorkAtAndSaveHandle_inlock(
    Date_t when,
    executor::TaskExecutor::CallbackFn work,
    executor::TaskExecutor::CallbackHandle* handle,
    const std::string& name) {
...
void GlobalInitialSyncer::_cancelHandle_inlock(executor::TaskExecutor::CallbackHandle handle) {
    if (!handle) {
        return;
    }
    _exec->cancel(handle);
...
Status GlobalInitialSyncer::_startupComponent_inlock(Component& component) {
    if (_isShuttingDown_inlock()) {
        component.reset();
        return Status(ErrorCodes::CallbackCanceled,
...
template <typename Component>
void GlobalInitialSyncer::_shutdownComponent_inlock(Component& component) {
    if (!component) {
        return;
    }
    component->shutdown();
...
/*
StatusWith<Operations> GlobalInitialSyncer::_getNextApplierBatch_inlock() {
    ...
    // See GlobalInitialSyncerOptions::getApplierBatchCallbackRetryWait.
...
*/
StatusWith<HostAndPort> GlobalInitialSyncer::_chooseSyncSource_inlock() {
...
Status GlobalInitialSyncer::_enqueueDocuments(Fetcher::Documents::const_iterator begin,
    ...
    log() << "MultiMaster GlobalSync _enqueueDocuments 1";
    if (info.toApplyDocumentCount == 0) {
        log() << "MultiMaster GlobalSync  _enqueueDocuments 2 no documents to apply";
    ...
    if (_isMongoG) {
        ...
        for (auto iDoc = begin; iDoc != end; ++iDoc) {
            log() << "MultiMaster GlobalSync mongoG _enqueueDocuments adding doc " << *iDoc;
            docs.emplace_back([&] {
        ...
        log() << "MultiMaster GlobalSync mongoG _enqueueDocuments Running command: " << cmd;
...
std::string GlobalInitialSyncer::Stats::toString() const {
    return toBSON().toString();
}

BSONObj GlobalInitialSyncer::Stats::toBSON() const {
    BSONObjBuilder bob;
    append(&bob);
    return bob.obj();
}

void GlobalInitialSyncer::Stats::append(BSONObjBuilder* builder) const {
...
std::string GlobalInitialSyncer::InitialSyncAttemptInfo::toString() const {
    return toBSON().toString();
}

BSONObj GlobalInitialSyncer::InitialSyncAttemptInfo::toBSON() const {
    BSONObjBuilder bob;
    append(&bob);
    return bob.obj();
}

void GlobalInitialSyncer::InitialSyncAttemptInfo::append(BSONObjBuilder* builder) const {
