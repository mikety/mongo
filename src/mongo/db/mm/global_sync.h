...
class GlobalSync {
    MONGO_DISALLOW_COPYING(GlobalSync);
...
public:
    ...
    /**
     * Constructs a GlobalSync to fetch oplog entries from a sync source.
...
private:
     ...
     * the producer thread uses the GlobalFetcher (which requires the replication coordinator
    ...
    /**
      ...
      *
      * (PR) Completely private to GlobalSync. Can be read or written to from within the main
      *      GlobalSync thread without synchronization. Shouldn't be accessed outside of this
      *      thread.
      *
    ...
    // Protects member data of GlobalSync.
    ...
    std::unique_ptr<GlobalFetcher> _oplogFetcher;
