...
Status checkRemoteOplogStart(const Fetcher::Documents& documents,
    ...
    */
    ...
    // OpTime to determine where to start our GlobalFetcher.
...
BSONObj GlobalFetcher::getMetadataObject_forTest() const {
    return _metadataObject;
}

Milliseconds GlobalFetcher::getAwaitDataTimeout_forTest() const {
    return _getGetMoreMaxTime();
}

Milliseconds GlobalFetcher::_getGetMoreMaxTime() const {
    return _awaitDataTimeout;
}

StatusWith<BSONObj> GlobalFetcher::_onSuccessfulBatch(const Fetcher::QueryResponse& queryResponse) {
