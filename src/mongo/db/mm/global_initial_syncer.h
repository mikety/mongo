...
MONGO_FAIL_POINT_DECLARE(failGlobalInitSyncWithBufferedEntriesLeft);

// Failpoint which causes the initial sync function to hang before copying databases.
MONGO_FAIL_POINT_DECLARE(globalInitialSyncHangBeforeCopyingDatabases);

// Failpoint which causes the initial sync function to hang before calling shouldRetry on a failed
// operation.
MONGO_FAIL_POINT_DECLARE(globalInitialSyncHangBeforeGettingMissingDocument);

// Failpoint which stops the applier.
MONGO_FAIL_POINT_DECLARE(rsGlobalSyncApplyStop);
...
struct GlobalInitialSyncerOptions {
    ...
    // GlobalInitialSyncer waits this long before retrying getApplierBatchCallback() if there are
    ...
    std::string toString() const {
        return str::stream() << "GlobalInitialSyncerOptions -- "
...
class GlobalInitialSyncer {
    MONGO_DISALLOW_COPYING(GlobalInitialSyncer);

public:
    /**
     * Callback function to report last applied optime of initial sync.
     */
    typedef stdx::function<void(const StatusWith<OpTime>&)> OnCompletionInitialSyncFn;
    ...
    struct Stats {
        ...
        std::vector<GlobalInitialSyncer::InitialSyncAttemptInfo> initialSyncAttemptInfos;
...
private:
     ...
     */
    void _setUp_inlock(std::uint32_t initialSyncMaxAttempts);

    /**
     * Tears down internal state before reporting final status to caller.
     */
    void _tearDown_inlock(const StatusWith<OpTime>& lastApplied);
    ...
    std::unique_ptr<GlobalFetcher> _globalFetcher;        // (S)
...
private:
    ...
    std::vector<std::unique_ptr<GlobalInitialSyncer>> _initialSyncers;
    std::vector<std::unique_ptr<GlobalSync>> _steadySyncers;
