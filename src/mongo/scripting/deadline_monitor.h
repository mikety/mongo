    ...
    ~DeadlineMonitor() {
        {
            ...
            stdx::lock_guard<std::mutex> lk(_deadlineMutex);
            _inShutdown = true;
...
private:
    ...
    void deadlineMonitorThread() {
        ...
        stdx::unique_lock<std::mutex> lk(_deadlineMutex);
    ...
    std::mutex _deadlineMutex;  // protects all non-const members, except _monitorThread
